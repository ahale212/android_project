...
public class MainDetails {
	...
	private static class DbHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL("CREATE TABLE IF NOT EXISTS " + DATABASE_USERS + " ( "  
					...
					+ KEY_HOSPITALNUMBER	+ " TEXT NOT NULL, "
	...
	public long createEntry(ArrayList<String> userDetails, int User_ID){
		...
		cv.put(KEY_USERID, User_ID);
		
		for(int i = 0; i<userDetails.size(); i++){
			cv.put(keys_UserDetails[i+1], userDetails.get(i));
		}
		...
		try {
			for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
				...
				if (cursor.getInt(cursor.getColumnIndex(KEY_USERID)) == User_ID){
					update =true;
				} else {
					update = false;
				}
	...
	if (update){
		return ourDatabase.update(DATABASE_TABLE, cv, KEY_ROWID + " = " + User_ID, null );
